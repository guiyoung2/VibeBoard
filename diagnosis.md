...
| **CI** | 없음 | GitHub Actions CI 추가 (`ci.yml`) | 신규 추가 |
| **번들 분석 도구** | 없음 | `rollup-plugin-visualizer` 추가 (`stats.html`) | 신규 추가 |
| **Lighthouse** | **미측정** | **6개 페이지 측정 완료** | **신규 측정** |
...
|--------|-------------|---------------|----------------|-----|-----|-----|-----|-----|
| `/` (메인) | **77** | 89 | 100 | 83 | **6.2s** | 0ms | 0 | 1.5s |
| `/games` (게임 목록) | **96** | 93 | 100 | 83 | **1.7s** | 0ms | 0.107 | 1.6s |
| `/games/:id` (게임 상세) | **62** | 95 | 100 | 83 | **6.5s** | 0ms | **0.295** | 1.5s |
| `/reviews` (후기 목록) | **75** | 87 | 100 | 83 | **5.9s** | 0ms | 0.107 | 1.6s |
| `/reviews/:id` (후기 상세) | **74** | 95 | 100 | 83 | **6.4s** | 0ms | 0.092 | 1.6s |
| `/cafes` (주변 매장) | **75** | 100 | 100 | 82 | **5.9s** | 0ms | 0.105 | 1.5s |

**주목 지점**:
- `/games` Performance **96점** — 정적 JSON 데이터 페이지라 LCP 1.7s로 가장 빠름
- `/games/:id` CLS **0.295** — Supabase 이미지 로드 중 레이아웃 이동 발생. CLS 0.1 미만이 목표치
...
| Lighthouse | 미측정 | 6개 페이지 측정 완료 | 성능 현황 파악, /games/:id CLS 개선 포인트 도출 |
